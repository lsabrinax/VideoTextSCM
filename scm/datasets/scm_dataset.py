...
from scm.utils.bbox_helper import *
...
class SubDataSet(object):
    ...
    def get_image_anno(self, video, track, frame):
        ...
        image_path = join(self.root, video, self.path_format.format(frame, track))
...
class DataSets(Dataset):
    def __init__(self, cfg, anchor_cfg, save_dir=None, num_epoch=1):
    ...
    def __getitem__(self, index, debug=False):
        ...
        template, tbbox, _ = self.template_aug(template_image, template_box, self.template_size, gray=gray)
        ...
        if debug:
            debug_dir = join(self.save_dir, 'debug')
            if not os.path.exists(debug_dir):
                os.makedirs(debug_dir)
            draw(template_image, template_box, join(debug_dir, "{:06d}_ot.jpg".format(index)))
            draw(search_image, search_box, join(debug_dir, "{:06d}_os.jpg".format(index)))
            draw(template, tbbox, join(debug_dir, "{:06d}_t.jpg".format(index)))
            draw(search, bbox, join(debug_dir, "{:06d}_s.jpg".format(index)))
            quit()
