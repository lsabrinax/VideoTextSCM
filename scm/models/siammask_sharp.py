...
class SiamMask(nn.Module):
    ...
    def run(self, template, search, softmax=False):
        template_feature = self.feature_extractor(template) #B * 256 * 7 * 7
        feature, search_feature = self.features.forward_all(search)#69/35/17/17
...
def select_mask_logistic_loss(p_m, mask, weight, o_sz=63, g_sz=127):
    ...
    loss = F.soft_margin_loss(p_m, mask_uf, reduction='none')
